...
async function init(cfg) {
    console.log('收到自定义参数:', JSON.stringify(cfg));
    
    // 示例：获取自定义参数
    if (cfg && cfg.anything) {
        console.log('anything 参数值:', cfg.anything);
    }

	Java.showAlertDialog('anything1',  cfg.anything);
    return {
        webview: {
            debug: true,
            showWebView: false,
...
async function categoryContent(tid, pg, filter, extend) {
    ...
    const document = await Java.wvOpen(`${baseUrl}/list/${type}-${area}-${sort}-${cat}-${lang}----${pg}---${year}.html`);
    const videos = parseVideoList(document);
