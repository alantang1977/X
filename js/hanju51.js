let host = 'https://www.hanju51.com';
let headers = {
...
function getList(html) {
    ...
    let items = container.querySelectorAll('.fed-list-item');  // 获取每个视频项

    items.forEach(it => {
        let title = it.querySelector('.fed-list-title')?.textContent.trim();
        let pic = it.querySelector('a .fed-list-pics')?.getAttribute('data-original');
        let remarks = it.querySelector('.fed-list-remarks')?.textContent.trim();
        let score = it.querySelector('.fed-list-score')?.textContent.trim();
        let link = it.querySelector('a')?.href;
